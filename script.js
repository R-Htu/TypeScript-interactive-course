          ...
          quiz: [
            {
              ...
              correct: 2,
              explain:
                "number[] is the shorthand syntax for an array of numbers.",
            },
            {
              q: "What does this declare?",
          ...
          quiz: [
            {
              ...
              correct: 2,
              explain:
                "interface defines reusable object shapes in TypeScript.",
            },
            {
      ...
      - Optional props should be marked with ?.`,
          ...
          quiz: [
            ...
            },
            {
              ...
              correct: 2,
              explain:
                "Use React.MouseEvent<T> to type mouse events on elements.",
            },
            {
...
// { name: string; age: number }`,
      ...
      // progress[lessonId] = { learn, quiz, code } booleans
      ...
      function lessonProgress(id) {
        return progress[id] || { learn: false, quiz: false, code: false };
      }
      function isLessonDone(id) {
        const p = lessonProgress(id);
        return p.learn && p.quiz && p.code;
      }
      ...
      function renderGrid() {
        updateStats();
        // Update each hardcoded card's lock/done state
        LESSONS.forEach((l, i) => {
          const card = document.getElementById("card-" + l.id);
          if (!card) return; // card-1 has no id set, handled separately
          ...
          card.onclick = unlocked ? () => openLesson(l.id) : null;

          const badge = card.querySelector(".l-badge");
          if (badge) {
            badge.className =
              "l-badge " + (done ? "done" : !unlocked ? "locked-b" : "open");
            badge.textContent = done ? "✔️" : !unlocked ? "🔒" : "[ >> ]";
          }
          ...
          dots.forEach(
            (d, di) => (d.className = "step-dot " + (states[di] || "")),
          );
        });

        // Also update card-1 (first card, no numeric id)
        const card1 = document.querySelector(
          ".lesson-grid .l-card:first-child",
        );
        if (card1) {
          const l = LESSONS[0];
          const done = isLessonDone(l.id);
          const p = lessonProgress(l.id);
          card1.className = "l-card " + (done ? "done" : "active-now");
          card1.onclick = () => openLesson(l.id);
          const badge = card1.querySelector(".l-badge");
          if (badge) {
            badge.className = "l-badge " + (done ? "done" : "open");
            badge.textContent = done ? "✔️" : "[ >> ]";
          }
          const dots = card1.querySelectorAll(".step-dot");
          const states = [
            p.learn ? "done" : "active",
            p.quiz ? "quiz-done" : p.learn ? "active" : "",
            p.code ? "code-done" : p.quiz ? "active" : "",
          ];
          dots.forEach(
            (d, di) => (d.className = "step-dot " + (states[di] || "")),
          );
      ...
      function openLesson(id) {
        ...
        // Always reset quiz state when opening a lesson
      ...
      function switchPhase(phase) {
        ...
        activePhase = phase;
        document.getElementById("phase-learn").style.display =
          phase === "learn" ? "block" : "none";
        document.getElementById("phase-quiz").style.display =
          phase === "quiz" ? "block" : "none";
        document.getElementById("phase-code").style.display =
          phase === "code" ? "block" : "none";
      ...
      function renderLearnPhase() {
        ...
        let html = `
    <div class="learn-lesson-hdr">
      <div class="learn-icon">${l.icon}</div>
      <div>
        <div class="learn-title">Lesson ${l.id} — ${l.title}</div>
        ...
        <div class="code-wrap">
          <div class="code-top">
            ...
            <span class="code-fname">${block.file || "example.ts"}</span>
          </div>
          <pre class="code-pre">${escHtml(block.code)}</pre>
        </div>`;
          }
          html += `</div>`;
        });
    ...
    <div class="quiz-hdr">
      ...
      <div class="prog-track" style="margin-bottom:16px">
      ...
      <div class="quiz-q-text">${q.q}</div>
        ...
        if (q.code) {
          html += `<div class="code-wrap" style="margin-bottom:12px">
      <div class="code-top"><div class="code-dots"><span class="r"></span><span class="y"></span><span class="g"></span></div></div>
      <pre class="code-pre">${escHtml(q.code)}</pre>
    </div>`;
        }
      ...
      <div class="choice" onclick="selectAnswer(${i})">
        ...
        <div class="choice-text">${escHtml(c)}</div>
      </div>`;
        });
        html += `</div>`;
        html += `<div class="feedback-box" id="quiz-feedback"></div>`;
    ...
    <div>
      ...
      function nextQuestion() {
        ...
        if (quizState.idx >= activeLesson.quiz.length) {
          finishQuiz();
        } else {
          renderQuestion();
        }
      ...
      function renderCodePhase() {
        ...
        let html = `
    <div class="challenge-hdr">
      <div class="challenge-title">⌨️ Code Challenge</div>
    ...
    <div class="editor-wrap">
      <div class="editor-label">Your TypeScript Code</div>
      <textarea class="code-editor" id="code-editor" spellcheck="false" placeholder="Write your TypeScript code here...">${done ? "// Already completed! Write more to practice.\n" : ch.starterCode}</textarea>
    </div>
    <div class="thinking" id="thinking">
      <span style="font-family:'JetBrains Mono',monospace">AI is checking your code</span>
    ...
    <div id="hint-box" style="display:none;margin-top:12px;padding:14px;background:rgba(251,191,36,.07);border:1px solid rgba(251,191,36,.2);border-radius:10px;font-family:'JetBrains Mono',monospace;font-size:.82rem;color:var(--yellow)">
      💡 ${ch.hint}
    </div>`;
      ...
      <div style="margin-top:16px">
        <button class="btn btn-primary btn-full" onclick="completionScreen()">
          Lesson Complete — Continue [ → ]
        </button>
      </div>`;
        }
      ...
      // ══════════════════════════════════════════
      // LOCAL CODE VALIDATOR
      // Smart regex-based checker — works offline,
      // no CORS issues, instant results
      // ══════════════════════════════════════════

      // Strip comments and normalize whitespace for checking
      ...
      // Check if code contains a variable declared with a specific type
      function hasTypedVar(code, name, type) {
        const c = stripComments(code);
        // matches: let/const/var name : type = ...
        // allows spaces around colon and optional semicolon
      ...
      // Check if code contains an interface definition
      function hasInterface(code, name) {
        const c = stripComments(code);
        return new RegExp(`interface\\s+${name}\\s*\\{`, "i").test(c);
      }

      // Check if code contains a function with typed params
      ...
      // Check if code contains a type alias
      ...
      // Check code contains a string array with values
      function hasStringArray(code, name) {
        const c = stripComments(code);
        // allow let/const/var, any spacing, single or double quotes don't matter
        return new RegExp(
      ...
      // Check code contains a tuple type
      function hasTuple(code, name, types) {
        const c = stripComments(code);
        // Don't escape the types string — it already contains \s* patterns
        return new RegExp(
      ...
      // Per-lesson validators — return { pass, title, feedback, hints }
      const VALIDATORS = {
        1: function (code) {
          ...
          if (hasUsername && hasYear)
            return {
              ...
              feedback:
                "You declared username as a string and year as a number — exactly right! Type annotations follow the pattern: let name: type = value. Well done!",
            };
          if (!hasUsername && !hasYear)
            return {
              pass: false,
              ...
              feedback:
                'Both variables need type annotations. Try:\nlet username: string = "YourName";\nlet year: number = 2026;',
            };
          if (!hasUsername)
            return {
              pass: false,
              title: "username type annotation missing",
              feedback:
                'year looks good! But username needs a : string annotation.\nFix: let username: string = "YourName";',
            };
          return {
            pass: false,
            title: "year type annotation missing",
            feedback:
              "username looks good! But year needs a : number annotation.\nFix: let year: number = 2026;",
          };
        },

        2: function (code) {
          const hasCity = hasTypedVar(code, "city", "string");
          const hasPopulation = hasTypedVar(code, "population", "number");
          const hasCapital = hasTypedVar(code, "isCapital", "boolean");

          const missing = [];
          if (!hasCity) missing.push("city: string");
          if (!hasPopulation) missing.push("population: number");
          if (!hasCapital) missing.push("isCapital: boolean");
          ...
          if (missing.length === 0)
            return {
              ...
              feedback:
                "You correctly typed all three primitive types: string, number, and boolean. These are the building blocks of TypeScript — great work!",
            };
          return {
            pass: false,
            title: `Missing: ${missing.join(", ")}`,
            feedback: `Still need these variables:\n${missing.map((m) => `  let ${m} = ...`).join("\n")}`,
          };
        },
        ...
        3: function (code) {
          ...
          const c = stripComments(code);
          // Count items — works with both " and ' quotes
          const fruitsMatch = c.match(
            /fruits\s*(?::\s*string\s*\[\]\s*)?=\s*\[([^\]]*)\]/i,
          );
          const fruitsContent = fruitsMatch ? fruitsMatch[1] : "";
          // Count commas+1 as a proxy for item count (works for any quote style)
          const fruitCount = fruitsContent.trim()
            ? (fruitsContent.match(/,/g) || []).length + 1
            : 0;

          if (hasFruits && hasPerson && fruitCount >= 3)
            return {
              pass: true,
              title: "Array and tuple both correct!",
              feedback:
                "Your string[] fruits array has 3+ items and the [string, number] person tuple is spot on. Tuples enforce both the length and the type at each position!",
            };
          if (!hasFruits && !hasPerson)
            return {
              pass: false,
              title: "Need a string[] array and a tuple",
              feedback:
                'Try:\nconst fruits: string[] = ["apple","banana","mango"];\nlet person: [string, number] = ["Alice", 25];',
            };
          if (!hasFruits || fruitCount < 3)
            return {
              pass: false,
              title: "fruits array needs 3+ items",
              feedback: `Use string[] type with at least 3 strings:\nconst fruits: string[] = ["apple","banana","mango"];${hasPerson ? "\n\nYour person tuple looks good!" : ""}`,
            };
          return {
            pass: false,
            title: "person tuple needs a type annotation",
            feedback: `Your fruits array looks good!\nThe tuple needs an explicit type:\nlet person: [string, number] = ["Alice", 25];\n\nMake sure the type annotation [string, number] is there.`,
          };
        },

        4: function (code) {
          const hasIface = hasInterface(code, "Product");
          const c = stripComments(code);
          const hasReadonlyId = /readonly\s+id\s*:\s*number/i.test(c);
          const hasName = /name\s*:\s*string/i.test(c);
          const hasPrice = /price\s*:\s*number/i.test(c);
          const hasVar = /:\s*Product/i.test(c);

          if (hasIface && hasReadonlyId && hasName && hasPrice && hasVar)
            return {
              pass: true,
              title: "Interface designed correctly!",
              feedback:
                "Your Product interface has readonly id, name, price — all correctly typed. And you created a variable using it. Interfaces are the backbone of TypeScript OOP!",
            };
          if (!hasIface)
            return {
              pass: false,
          ...
          const missing = [];
          if (!hasReadonlyId) missing.push("readonly id: number");
          if (!hasName) missing.push("name: string");
          if (!hasPrice) missing.push("price: number");
          if (!hasVar) missing.push("a variable of type Product");
          return {
            pass: false,
            title: "Interface incomplete",
            feedback: `Interface found! Still missing:\n${missing.map((m) => "  • " + m).join("\n")}`,
          };
        },

        5: function (code) {
          const hasFunc = hasFunction(code, "formatName");
          const c = stripComments(code);
          const hasFirstName = /firstName\s*:\s*string/i.test(c);
          const hasLastName = /lastName\s*:\s*string/i.test(c);
          const hasReturn = /\)\s*:\s*string\s*[{=]/i.test(c);
          const hasOptional = /title\s*\?\s*:\s*string/i.test(c);

          if (hasFunc && hasFirstName && hasLastName && hasReturn)
            return {
              pass: true,
              title: "Function typed correctly!",
              feedback: `Great work! formatName has typed parameters and a string return type.${hasOptional ? " And the optional title parameter is a nice touch!" : " Try adding an optional title?: string parameter for extra credit!"}`,
            };
          if (!hasFunc)
            return {
              pass: false,
              title: "No formatName function found",
              feedback:
                "Write a function named formatName:\nfunction formatName(firstName: string, lastName: string, title?: string): string {\n  return `${firstName} ${lastName}`;\n}",
            };
          const missing = [];
          if (!hasFirstName) missing.push("firstName: string parameter");
          if (!hasLastName) missing.push("lastName: string parameter");
          if (!hasReturn) missing.push("): string return type annotation");
          return {
            pass: false,
            title: "Function needs more types",
            feedback: `Function found! Still missing:\n${missing.map((m) => "  • " + m).join("\n")}`,
          };
        },

        6: function (code) {
          const hasResult = hasTypeAlias(code, "Result", null);
          const hasTheme = hasTypeAlias(code, "Theme", null);
          const c = stripComments(code);

          // Check Result contains union of 3 types
          const resultMatch = c.match(/type\s+Result\s*=([^;]+)/i);
          const resultHasAll =
            resultMatch &&
            /string/i.test(resultMatch[1]) &&
            /number/i.test(resultMatch[1]) &&
            /boolean/i.test(resultMatch[1]);

          // Check Theme is a literal union
          const themeMatch = c.match(/type\s+Theme\s*=([^;]+)/i);
          const themeHasLiterals =
            themeMatch &&
            /"light"/i.test(themeMatch[1]) &&
            /"dark"/i.test(themeMatch[1]);

          const hasThemeVar = /:\s*Theme/i.test(c);

          if (resultHasAll && themeHasLiterals && hasThemeVar)
            return {
              pass: true,
              title: "Union types mastered!",
              feedback:
                'Your Result union covers string | number | boolean, Theme is a proper literal type "light" | "dark", and currentTheme is correctly typed. Literal types are one of TypeScript\'s most powerful features!',
            };

          const missing = [];
          if (!hasResult || !resultHasAll)
            missing.push("Result = string | number | boolean");
          if (!hasTheme || !themeHasLiterals)
            missing.push('Theme = "light" | "dark"');
          if (!hasThemeVar) missing.push("a variable typed as Theme");
          return {
            pass: false,
        ...
        7: function (code) {
          const c = stripComments(code);
          const hasClass = /class\s+Car\s*\{/i.test(c);
          const hasMake = /(?:public\s+)?make\s*:\s*string/i.test(c);
          const hasModel = /(?:public\s+)?model\s*:\s*string/i.test(c);
          const hasYear = /(?:private\s+)?year\s*:\s*number/i.test(c);
          const hasMethod = /getInfo\s*\(\s*\)\s*:\s*string/i.test(c);
          if (hasClass && hasMake && hasModel && hasYear && hasMethod)
            return {
              pass: true,
              title: "Class built correctly!",
              feedback:
                "Your Car class has typed properties (make, model, year) and a getInfo(): string method. You've got TypeScript classes down!",
            };
          if (!hasClass)
            return {
              pass: false,
          ...
          const missing = [];
          if (!hasMake) missing.push("public make: string");
          if (!hasModel) missing.push("public model: string");
          if (!hasYear) missing.push("private year: number");
          if (!hasMethod) missing.push("getInfo(): string method");
          return {
            pass: false,
            title: "Class incomplete",
            feedback: `Class found! Still missing:\n${missing.map((m) => "  • " + m).join("\n")}`,
          };
        },

        8: function (code) {
          const c = stripComments(code);
          const hasGenericFn = /function\s+wrapInArray\s*<\s*T\s*>/i.test(c);
          const hasParamT = /value\s*:\s*T\b/i.test(c);
          const hasReturnT = /\)\s*:\s*T\s*\[/i.test(c);
          const hasCalls = (c.match(/wrapInArray\s*[(<]/g) || []).length >= 2;
          if (hasGenericFn && hasParamT && hasReturnT && hasCalls)
            return {
              pass: true,
              title: "Generic function perfect!",
              feedback:
                "wrapInArray<T> correctly takes T and returns T[]. Calling it twice with different types proves the generic works. Excellent!",
            };
          if (!hasGenericFn)
            return {
              pass: false,
              ...
              feedback:
                "Use <T> after the function name:\nfunction wrapInArray<T>(value: T): T[] { return [value]; }",
            };
          const missing = [];
          if (!hasParamT) missing.push("parameter typed as T");
          if (!hasReturnT) missing.push("return type T[]");
          if (!hasCalls) missing.push("at least 2 calls with different types");
          return {
            pass: false,
            title: "Almost there!",
            feedback: `Function found! Still missing:\n${missing.map((m) => "  • " + m).join("\n")}`,
        ...
        9: function (code) {
          ...
          const hasSpring = /Spring\s*=\s*["']spring["']/i.test(c);
          const hasSummer = /Summer\s*=\s*["']summer["']/i.test(c);
          const hasAutumn = /Autumn\s*=\s*["']autumn["']/i.test(c);
          const hasWinter = /Winter\s*=\s*["']winter["']/i.test(c);
          const hasVar = /:\s*Season/i.test(c);
          if (
            hasEnum &&
            hasSpring &&
            hasSummer &&
            ...
            return {
              pass: true,
              title: "Enum defined perfectly!",
              feedback:
                "Season has all four string members and currentSeason is correctly typed. String enums are the preferred pattern in modern TypeScript!",
            };
          if (!hasEnum)
            return {
              pass: false,
          ...
          const missing = [];
          if (!hasSpring) missing.push('Spring = "spring"');
          if (!hasSummer) missing.push('Summer = "summer"');
          if (!hasAutumn) missing.push('Autumn = "autumn"');
          if (!hasWinter) missing.push('Winter = "winter"');
          if (!hasVar) missing.push("a variable typed as Season");
          return {
            pass: false,
        ...
        10: function (code) {
          const c = stripComments(code);
          const hasFn = /function\s+describe\s*\(/i.test(c);
          const hasUnion = /string\s*\|\s*number\s*\|\s*boolean/i.test(c);
          const hasTypeofS = /typeof\s+\w+\s*===?\s*["']string["']/i.test(c);
          const hasTypeofN = /typeof\s+\w+\s*===?\s*["']number["']/i.test(c);
          const hasReturn = /return/i.test(c);
          if (hasFn && hasUnion && hasTypeofS && hasTypeofN && hasReturn)
            return {
              pass: true,
              title: "Type narrowing mastered!",
              feedback:
                "describe() correctly narrows string | number | boolean using typeof guards. TypeScript knows exactly which type you're working with in each branch!",
            };
          if (!hasFn)
            return {
              pass: false,
          ...
          const missing = [];
          if (!hasUnion) missing.push("string | number | boolean union type");
          if (!hasTypeofS) missing.push('typeof check for "string"');
          if (!hasTypeofN) missing.push('typeof check for "number"');
          return {
            pass: false,
        ...
        11: function (code) {
          ...
          const hasExportIface = /export\s+interface\s+Person\s*\{/i.test(c);
          const hasName = /name\s*:\s*string/i.test(c);
          const hasAge = /age\s*:\s*number/i.test(c);
          const hasImport =
            /import\s*\{[^}]+\}\s*from\s*["']\.\/utils["']/i.test(c);
          if (hasExportFn && hasExportIface && hasName && hasAge && hasImport)
            return {
              pass: true,
              title: "Modules understood!",
              feedback:
                "You exported greet with typed params, exported Person interface, and wrote the import. You've got the full export/import cycle mastered!",
            };
          const missing = [];
          if (!hasExportFn)
            missing.push("export function greet(name: string): string");
          if (!hasExportIface) missing.push("export interface Person { ... }");
          if (!hasName) missing.push("name: string in Person");
          if (!hasAge) missing.push("age: number in Person");
          if (!hasImport)
            missing.push('import { greet, Person } from "./utils"');
          return {
            pass: false,
        ...
        12: function (code) {
          const c = stripComments(code);
          const hasOmit = /Omit\s*<\s*Product\s*,/i.test(c);
          const hasPartial = /Partial\s*<\s*PublicProduct\s*>/i.test(c);
          const hasPublic = /type\s+PublicProduct/i.test(c);
          const hasUpdate = /type\s+UpdateProduct/i.test(c);
          if (hasOmit && hasPartial && hasPublic && hasUpdate)
            return {
              pass: true,
              title: "Utility types composed!",
              feedback:
                "PublicProduct uses Omit to strip the secret field, and UpdateProduct wraps it in Partial. This is exactly how real TypeScript projects protect sensitive data!",
            };
          const missing = [];
          if (!hasPublic || !hasOmit)
            missing.push('type PublicProduct = Omit<Product, "secret">');
          if (!hasUpdate || !hasPartial)
            missing.push("type UpdateProduct = Partial<PublicProduct>");
          return {
            pass: false,
        ...
        13: function (code) {
          const c = stripComments(code);
          const hasDecoFn =
            /function\s+Readonly\s*\(\s*\w+\s*:\s*Function\s*\)/i.test(c);
          const hasFreeze = /Object\.freeze\s*\(/i.test(c);
          const hasClass = /class\s+Config/i.test(c);
          const hasApplied = /@Readonly/i.test(c);
          const hasVersion = /version\s*:\s*string/i.test(c);
          if (hasDecoFn && hasFreeze && hasClass && hasApplied && hasVersion)
            return {
              pass: true,
              title: "Decorator written correctly!",
              feedback:
                "Your Readonly decorator takes a Function constructor, freezes the prototype, and is applied to Config with @Readonly. That's a proper TypeScript decorator!",
            };
          const missing = [];
          if (!hasDecoFn)
            missing.push("function Readonly(constructor: Function) { ... }");
          if (!hasFreeze)
            missing.push("Object.freeze() call inside the decorator");
          if (!hasClass) missing.push("class Config { ... }");
          if (!hasApplied) missing.push("@Readonly applied above the class");
          if (!hasVersion) missing.push("version: string property in Config");
          return {
            pass: false,
        ...
        14: function (code) {
          ...
          const hasOutDir = /outDir\s*:\s*["']\.\/dist["']/i.test(c);
          const hasRootDir = /rootDir\s*:\s*["']\.\/src["']/i.test(c);
          if (hasTarget && hasStrict && hasOutDir && hasRootDir)
            return {
              pass: true,
              title: "tsconfig configured correctly!",
              feedback:
                "All four settings are present: ES2021 target, strict mode, ./dist output, and ./src root. This is a solid production-ready TypeScript config!",
            };
          const missing = [];
          if (!hasTarget) missing.push('target: "ES2021"');
          if (!hasStrict) missing.push("strict: true");
          if (!hasOutDir) missing.push('outDir: "./dist"');
          if (!hasRootDir) missing.push('rootDir: "./src"');
          return {
            pass: false,
            title: "Config incomplete",
            feedback: `Missing settings:\n${missing.map((m) => "  • " + m).join("\n")}`,
          };
        },

        15: function (code) {
          const c = stripComments(code);
          const hasIface = /interface\s+CardProps\s*\{/i.test(c);
          const hasTitle = /title\s*:\s*string/i.test(c);
          const hasDesc = /description\s*:\s*string/i.test(c);
          const hasClick = /onClick\s*\?\s*:\s*\(\s*\)\s*=>\s*void/i.test(c);
          const hasFn = /function\s+Card\s*\(|const\s+Card\s*=/i.test(c);
          const usesProps = /CardProps/i.test(
            c.replace(/interface\s+CardProps/i, ""),
          );
          if (hasIface && hasTitle && hasDesc && hasClick && hasFn && usesProps)
            return {
              pass: true,
              title: "React component typed!",
              feedback:
                "CardProps has all three properties correctly typed, and Card uses it as the parameter type. This is exactly how modern React + TypeScript components are written!",
            };
          const missing = [];
          if (!hasIface) missing.push("interface CardProps { ... }");
          if (!hasTitle) missing.push("title: string");
          if (!hasDesc) missing.push("description: string");
          if (!hasClick) missing.push("onClick?: () => void");
          if (!hasFn) missing.push("function Card component");
          if (!usesProps) missing.push("CardProps used as parameter type");
          return {
            pass: false,
        ...
        16: function (code) {
          const c = stripComments(code);
          const hasIface = /interface\s+PostParams\s*\{/i.test(c);
          const hasIdStr = /id\s*:\s*string/i.test(c);
          const hasFn = /getPost/i.test(c);
          const hasReqType = /Request\s*<\s*PostParams\s*>/i.test(c);
          const hasRes = /res\s*:\s*Response/i.test(c);
          const hasParamId = /req\.params\.id/i.test(c);
          const hasJson = /res\.json\s*\(/i.test(c);
          if (
            hasIface &&
            hasIdStr &&
            hasFn &&
            ...
            return {
              pass: true,
              title: "Express route fully typed!",
              feedback:
                "PostParams interface, typed Request<PostParams>, accessing req.params.id, and res.json() — all correct. This is production-grade Express TypeScript!",
            };
          const missing = [];
          if (!hasIface) missing.push("interface PostParams { id: string }");
          if (!hasReqType) missing.push("Request<PostParams> type on req");
          if (!hasRes) missing.push("res: Response parameter");
          if (!hasParamId) missing.push("req.params.id access");
          if (!hasJson) missing.push("res.json() call");
          return {
            pass: false,
        ...
        17: function (code) {
          const c = stripComments(code);
          const hasConditional = /type\s+IsArray\s*<\s*T\s*>/i.test(c);
          const hasExtends = /T\s+extends\s+any\s*\[\]/i.test(c);
          const hasTernary = /\?\s*["']yes["']\s*:\s*["']no["']/i.test(c);
          const hasTest1 = /type\s+Test1/i.test(c);
          const hasTest2 = /type\s+Test2/i.test(c);
          if (
            hasConditional &&
            hasExtends &&
            hasTernary &&
            ...
            return {
              pass: true,
              title: "Conditional type mastered!",
              feedback:
                'IsArray<T> uses T extends any[] ? "yes" : "no" — the correct pattern. Test1 and Test2 verify it works for both cases. You\'ve completed all 17 lessons — congratulations! 🎉',
            };
          const missing = [];
          if (!hasConditional) missing.push("type IsArray<T> = ...");
          if (!hasExtends) missing.push("T extends any[] condition");
          if (!hasTernary) missing.push('? "yes" : "no" result types');
          if (!hasTest1) missing.push("type Test1 = IsArray<...array type...>");
          if (!hasTest2)
            missing.push("type Test2 = IsArray<...non-array type...>");
          return {
            pass: false,
      ...
      function submitCode() {
        ...
        // Show "checking" state briefly for UX feel
        ...
        // Small delay so it feels like it's "checking"
        setTimeout(() => {
          ...
          if (result.pass) {
            const id = activeLesson.id;
            const wasAlreadyDone = lessonProgress(id).code;
        ...
        <div class="ai-result-hdr">
          <div class="ai-result-icon">(^_^)</div>
          <div class="ai-result-title">${escHtml(result.title)}</div>
        </div>
        <div class="ai-result-body">${escHtml(result.feedback)}</div>
        <div class="btn-row" style="margin-top:16px">
          <button class="btn btn-primary" onclick="completionScreen()">
            Complete Lesson! [ \\o/ ]
          </button>
        </div>`;
          } else {
            ...
            resultEl.innerHTML = `
        <div class="ai-result-hdr">
          <div class="ai-result-icon">(x_x)</div>
          <div class="ai-result-title">${escHtml(result.title)}</div>
        </div>
        <div class="ai-result-body">${escHtml(result.feedback)}</div>
        <div class="btn-row" style="margin-top:16px">
          <button class="btn btn-ghost" onclick="submitCode()">Try Again [ → ]</button>
        </div>`;
          }
        }, 900); // 0.9s "checking" animation
      }
    ...
    <div class="complete-sub">You finished: Learn → Quiz → Code Challenge</div>
    <div class="complete-rewards">
      <div class="reward-card">
        <div class="reward-val" style="color:var(--yellow)">(*)</div>
        <div class="reward-val" style="color:var(--yellow)">${points}</div>
        <div class="reward-label">Total Points</div>
      </div>
      <div class="reward-card">
        <div class="reward-val" style="color:var(--orange)">>_<</div>
        <div class="reward-val" style="color:var(--orange)">${streak}</div>
        <div class="reward-label">Streak</div>
      </div>
      <div class="reward-card">
        <div class="reward-val" style="color:var(--cyan)">[@]</div>
        <div class="reward-val" style="color:var(--cyan)">${LESSONS.filter((x) => isLessonDone(x.id)).length}/${LESSONS.length}</div>
        <div class="reward-label">Lessons Done</div>
      </div>
    </div>
    <div class="btn-row" style="justify-content:center">
      ${
        hasNext
          ? `<button class="btn btn-primary" onclick="openLesson(${LESSONS[nextIdx].id})">
        Next Lesson: ${LESSONS[nextIdx].title} [ → ]
      </button>`
          : `<div style="font-family:'JetBrains Mono',monospace;color:var(--cyan);font-size:1.1rem">
        (^_^) You completed all lessons!
      </div>`
      }
